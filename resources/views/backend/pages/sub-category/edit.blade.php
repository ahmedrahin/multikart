...
@section('page-title')
    <title>Update Sub-category Information || {{ !is_null($siteTitle = App\Models\Settings::site_title()) ? $siteTitle->site_title : 'Shop' }}</title>
@endsection
        ...
        <div class="row">
            <div class="col-12 d-flex">
                ...
                <div class="card-body">
                    ...
                    <h5 class="heading">Update Category Information</h5>
                    </div>
                    
                    <div class=" mb-3 border p-3 radius-10">
                         <!-- Message -->
                         @include('backend.includes.message')
                        <form action="{{ route('update-subCategory', $editData->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            ...
                            <div class="row form-row g-lg-5">
                                <div class="col-lg-6 col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" name="name" id="name" placeholder="Enter Category Name" value="{{ $editData->name }}" class="@error('name') is-invalid @enderror">
                                        @error('name')
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select name="category_id" id="" class="select2 @error('category_id') is-invalid @enderror">
                                            ...
                                            @foreach( $parentCategories as $parentCategory )
                                                <option value="{{ $parentCategory->id }}" @if( $editData->category_id == $parentCategory->id ) selected @endif >
                                                    {{ $parentCategory->name }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <div class="row">
                                            <div class="col-lg-8 col-md-8">
                                                ...
                                                @if( !is_null($editData->image) )
                                                    <div class="img-box">
                                                        <div class="has-img">
                                                            <img src="{{ asset('uploads/sub-categories/' . $editData->image) }}" alt="" id="upImg" >
                                ...
                                <div class="col-lg-6 col-md-6">
                                     <div class="mb-4">
                                        ...
                                        <textarea name="description" id="description" placeholder="Write Description..">{{ $editData->description }}</textarea>
                                    </div>
                                    <div>
                        ...
                        @if( !is_null($editData->image) )
                            <form action="{{ route('remove-subcategory-image', ['id' => $editData->id]) }}" method="POST" name="remove-img" class="remove-img" id="removeImg">
                                @csrf
                                @method('PATCH')
...
@section('page-script')
	...
	<script>
        ...
        btnRemove.addEventListener('click', function(){
            ...
            if(removeImage){
                img.src = "{{ asset('uploads/sub-categories/' . $editData->image) }}";
                removeImage.style.zIndex = "1";
            }
        })
	</script>
            ...
            $('#removeImg').on('submit', function(e){
                ...
                $.ajax({
                    ...
                    success: function(response){
                        toastr.error('Category Image is Deleted');
